    ...
    def run(self, threshold=0.15):
        ...
        while self.can_run.is_set():
            if self.sensor_rx.poll():
                ...
                history.append(data)
                if len(history) == 5:
                    previous_data = history[0]
                    if is_stationary(data, previous_data, threshold):
                        label = "fermo"
                    else:
                        label = "movimento"
                    
                print(f"accX:{accX:.2f} accY:{accY:.2f} accZ:{accZ:.2f} gyroX:{gyroX:.2f} gyroY:{gyroY:.2f} gyroZ:{gyroZ:.2f}, stato:{label}")
